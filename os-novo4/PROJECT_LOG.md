...
| `scheduler.hpp` / `scheduler.cpp` | Ready red (intruzivna kružna lista), `schedulerRun`, `schedulerTick`, `schedulerAddThread`, `schedulerRequestStopAfter`, `schedulerYieldFinished` — preemptivni round-robin |
...
### Test status
Trenutno **9/9 testova prolazi** (`make clean && make run`):
- UART ispis
- CSR čitanje (`mstatus`)
...
- **Preemptivni scheduler:** dve niti sa beskonačnim petljama, oba brojača
  rastu bez ijednog dobrovoljnog ustupanja procesora — dokaz da prinudno
  preotimanje radi
- Trap/ecall: `mcause` tačno odgovara `EcallFromMMode`

(Napomena: broj testova je pao sa 10 na 9 u odnosu na korak 2 — stari
"niti se smenjuju ABABAB" test je zamenjen jednim testom preemptivnog
schedulera, ne dodat uz njega, jer stari kooperativni `switchInto`/
`yieldToMain` API više ne postoji.)
...
*Poslednje ažurirano: posle koraka 3 (preemptivni round-robin scheduler),
pre koraka 4 (semafori).*
