...
int main() {
    // Sto ranije, da uhvati BAS SVE scheduler/semafor dogadjaje kroz ceo
    // ostatak main()-a (preemptivni scheduler, semafori, SMP test) u jedan
    // kumulativni trag, umesto da se pravi posebna, vestacka scena samo za
    // ovaj test.
    traceInit();
    ...
    printString("\n[Profiling trag]\n");
    {
        // Trag je vec pun dogadjaja iz SVIH prethodnih testova (preemptivni
        // scheduler, semafori, SMP paralelizam) -- ovde ga samo ocenjujemo i
        // ispisujemo, ne pravimo novu vestacku scenu.
        const int count = traceCount();

        printString("  Zabelezeno dogadjaja: ");
        printDecimal(static_cast<unsigned>(count));
        printString("\n");

        tests.check("trag sadrzi razuman broj zabelezenih dogadjaja (> 10)",
                    count > 10);
        tests.check(
            "trag sadrzi dogadjaje sa OBA harta (dokaz da profiler stvarno "
            "vidi SMP aktivnost, ne samo harta 0)",
            traceHasEventOnHart(0) && traceHasEventOnHart(1));

        // Pun CSV dump -- nalepi ovaj deo izlaza (izmedju TRACE START/END
        // markera) u tools/trace-viewer.html da vidis Gantt prikaz.
        traceDump();
    }
    ...
    RISCV::Poweroff::exitQemu(tests.passed == tests.total ? RISCV::Poweroff::Pass
                                                            : RISCV::Poweroff::Fail);
}
