...
void schedulerRun() {
    ...
    while (!stopRequested) {
        ...
        traceRecord(TraceEvent::Dispatch, next->id);
...
void schedulerTick() {
    ...
    traceRecord(TraceEvent::Preempted, preempted->id);
...
void schedulerYieldFinished() {
    ...
    currentTcb[hart] = nullptr;
    traceRecord(TraceEvent::Finished, self->id);
...
void schedulerWake(TCB* thread) {
    ...
    traceRecord(TraceEvent::Woken, thread->id);
